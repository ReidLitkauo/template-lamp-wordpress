...
]; ?><?php ############################################################################### ?><?php # /src/$-template-bridge.pug ?><?php # This file provides the bridge between the framework's render_template() ?><?php # and actually showing some HTML. ?><?php # Provides a basic framework for all pages for the site. ?><html><head><script<?= (is_bool($_pug_temp = $pugModule['Phug\\Formatter\\Format\\BasicFormat::attributes_assignment']([], ['src' => $pugModule['Phug\\Formatter\\Format\\BasicFormat::array_escape']('src', "/static/".(isset($endpoint) ? $endpoint : null).".js")])) ? var_export($_pug_temp, true) : $_pug_temp) ?>></script><link<?= (is_bool($_pug_temp = $pugModule['Phug\\Formatter\\Format\\BasicFormat::attributes_assignment']([], ['rel' => 'stylesheet'], ['href' => $pugModule['Phug\\Formatter\\Format\\BasicFormat::array_escape']('href', "/static/".(isset($endpoint) ? $endpoint : null).".css")])) ? var_export($_pug_temp, true) : $_pug_temp) ?> /><?php require_once "{$_SERVER['DOCUMENT_ROOT']}/../templates/$endpoint.head.php" ?></head><body><main><?php require_once "{$_SERVER['DOCUMENT_ROOT']}/../templates/$endpoint.body.php" ?></main></body></html>